...
uint8_t HugoUI::Transition_Hourglass(void)
{
    ...
    static uint8_t _animation_status = 0;
    ...
    for (int16_t i = 0; i <= SCREEN_WIDTH; i += 2)
        for (int16_t j = _temp_h - 6; j <= _temp_h - 1; j++)
        {
            if (j % 2 == 0)
    ...
    Animation_Linear(&_temp_h, &_temp_h_trg, 75);

    // 下落过程
    if (_animation_status == 0 && _temp_h == _temp_h_trg && _temp_h == SCREEN_HEIGHT + 8)
    {
        _animation_status = 1; // 落下来了
        return _animation_status;
    }
    // 上面_animation_status=1之后 return了 进到core里刷新了背景显示内容 下一次进到本函数就可以把标志位置为2
    if (_animation_status == 1)
    {
        // _temp_h_trg = SCREEN_HEIGHT + 8;
        _temp_h_trg = -8;      // 使其开始上升
        _animation_status = 2; // 开始抬起
        return _animation_status;
    }

    if (_animation_status == 2 && _temp_h <= -1)
    {
        _animation_status = 0; // 退场动画完成
        _temp_h = -8;
        _temp_h_trg = SCREEN_HEIGHT + 8;
        return _animation_status;
    }
